...
class Chain extends AsyncEmitter {
  ...
  async getSuitableBlock(prev) {
    ...
    blocks[2] = await this.getPrevious(blocks[1]);

    blocks.sort((a, b) => a.bits - b.bits);
  ...
  computeTarget(first, last) {
    ...
    target.isub(consensus.fromCompact(first.bits));
    target.imuln(pow.retargetInterval);
    ...
    target.idivn(actualTimespan);

    return target.neg().idivn(target);
  }
