...
class Packet {
  ...
  isBlockLike() {
    return isBlockLike(this.type);
  }

  /**
   * Check if packet size is oversized.
   * @param {Number} maxBlockSize
   * @returns {Boolean}
   */

  isOversized(maxBlockSize) {
    return isOversized(this.type, this.getSize(), maxBlockSize);
  }
}
...
function isBlockLike(packetType) {
  let type;
  ...
  switch (type) {
    case exports.types.BLOCK:
    case exports.types.CMPCTBLOCK:
    case exports.types.BLOCKTXN:
      return true;
  }
...
function isOversized(packetType, size, maxBlockSize) {
  if (!maxBlockSize)
    maxBlockSize = consensus.MAX_FORK_BLOCK_SIZE;
...
exports.isOversized = isOversized;
