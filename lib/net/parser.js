...
class Parser extends EventEmitter {
   ...
   * @param {Function} maxBlockSize
   */

  constructor(network, maxBlockSize) {
    super();
  ...
  parse(data) {
    assert(data.length <= consensus.MAX_FORK_BLOCK_SIZE * 2);
  ...
  parseHeader(data) {
    ...
    if (packets.isOversized(cmd, size, this.maxBlockSize())) {
      this.waiting = 24;
