...
class Peer extends EventEmitter {
  ...
  constructor(options) {
    ...
    this.parser = new Parser(this.network, this.options.maxBlockSize);
  ...
  send(packet) {
    ...
    assert(!packet.isOversized(this.options.maxBlockSize()),
      'Packet is oversized.');
...
class PeerOptions {
  ...
  constructor(options) {
    ...
    this.maxBlockSize = PeerOptions.maxBlockSize;
  ...
  fromOptions(options) {
    ...
    if (options.maxBlockSize != null) {
      assert(typeof options.maxBlockSize === 'function');
  ...
  static maxBlockSize() {
    return consensus.MAX_FORK_BLOCK_SIZE;
  }
}
