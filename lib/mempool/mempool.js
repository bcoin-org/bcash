...
class MempoolCache {
  ...
  async getTip() {
    const hash = await this.db.get(layout.R.encode());
  ...
  sync(tip) {
    ...
    this.batch.put(layout.R.encode(), Buffer.from(tip, 'hex'));
  }
  ...
  async init(hash) {
    ...
    batch.put(layout.R.encode(), Buffer.from(hash, 'hex'));
    await batch.write();
  }
  ...
  async wipe() {
    ...
    batch.put(layout.v.encode(), fromU32(MempoolCache.VERSION));
    batch.put(layout.R.encode(), Buffer.from(this.chain.tip.hash, 'hex'));
