...
Address.prototype.fromRaw = function fromRaw(data, network) {
  ...
  switch (prefix) {
    case 0x1c:
      ...
      assert(!network || Network.get(network) === Network.get('main'));
      network = Network.get('main');
      break;
    case 0x28:
      type = Address.types.SCRIPTHASH;
      assert(!network || Network.get(network) === Network.get('main'));
      network = Network.get('main');
      break;
    default:
  ...
  if (type === Address.types.WITNESS) {
    ...
    const size = br.left() - 4;

    if (size < 2 || size > 40)
      throw new Error('Bad program size.');
  }
