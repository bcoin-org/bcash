...
class WalletDB extends EventEmitter {
  ...
  async open() {
    await this.db.open();
    await this.db.verify(layout.V.encode(), 'wallet', 0x80 | 7);
  ...
  async _addBlock(entry, txs) {
    ...
    // tx -> deps
    const hashes = new BufferMap();

    for (const tx of txs) {
      if (tx.isCoinbase())
        continue;

      hashes.set(tx.hash(), 0);
    }
    ...
    for (const tx of txs) {
      ...
      for (const {prevout} of tx.inputs) {
        const phash = prevout.hash;

        if (!hashes.has(phash))
          continue;

        hashes.set(thash, hashes.get(thash) + 1);

        if(!depMap.has(phash))
          depMap.set(phash, []);

        depMap.get(phash).push(tx);
      }

      if (hashes.get(thash) > 0)
        continue;
    ...
    while (queue.length > 0) {
      ...
      for (const tx of deps) {
        let tdeps = hashes.get(tx.hash());

        tdeps -= 1;
        hashes.set(tx.hash(), tdeps);
