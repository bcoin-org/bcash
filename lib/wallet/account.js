...
Account.prototype.deriveKey = function deriveKey(branch, index, master) {
  ...
  if (master && master.key && !this.watchOnly) {
    key = master.key.deriveBIP45();
    key = key.derive(0x80000000 - 1).derive(branch).derive(index);
  } else {
    key = this.accountKey.derive(0x80000000 - 1).derive(branch).derive(index);
  }
  ...
  switch (this.type) {
    ...
    case Account.types.MULTISIG:
      ...
      for (const shared of this.keys) {
        const key = shared.derive(0x80000000 - 1).derive(branch).derive(index);
        keys.push(key.publicKey);
      }
